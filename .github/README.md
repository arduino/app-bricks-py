...
The notes of the GitHub Release are generated by GitHub from the changes since the previous release of
the library, which `scripts/previous_release_tag.py` picks by version among the `release/*` tags and the
`bricks/*` tags of the period when the library was released under that prefix. Left to itself, GitHub
would start from the latest release of any kind, and the prebuilt binaries (`llamacpp/*`, `opencv/*`,
`libcamera/*`) are published as releases too.
