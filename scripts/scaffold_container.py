...
def bake_target_text(name: str, parent: str | None) -> str:
...
def add_bake_target(hcl: str, name: str, parent: str | None, containers: Containers) -> str:
    ...
    hcl = hcl[:end] + "\n" + bake_target_text(name, parent) + hcl[end:]
...
def add_licensed_app(config: str, name: str) -> str:
...
def add_dependabot_directory(config: str, name: str) -> str:
    """Add the container's uv project to the directories Dependabot updates."""
    entry = f"      - /containers/{name}\n"
    if entry in config:
        raise ScaffoldError(f".github/dependabot.yml already lists /containers/{name}.")
    entries = list(re.finditer(r"^      - /containers/[a-z0-9-]+\n", config, re.MULTILINE))
    if not entries:
...
def scaffold(repo_root: Path, name: str, parent_or_image: str, desc: str, python: bool) -> list[str]:
    ...
    if python:
        ...
        updates[dependabot] = add_dependabot_directory(dependabot.read_text(encoding="utf-8"), name)
    ...
    steps = [f"Complete containers/{name}/Dockerfile and the purpose of '{name}' in containers/README.md."]
    if python:
        steps.append(f"Declare the packages in containers/{name}/pyproject.toml, then run `task deps:lock` and `task license:deps`.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print(f"Scaffolded containers/{args.name}. Next steps:")
