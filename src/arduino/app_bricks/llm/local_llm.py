...
class LargeLanguageModel(CloudLLM):
    ...
    def chat(self, message: str, images: List[str | bytes] = None, reasoning: bool = False) -> str | SimpleNamespace:
        ...
        Args:
            ...
            reasoning (bool): Whether to extract reasoning from the response if present.

        Returns:
            str | SimpleNamespace: The complete text response generated by the AI
                or a SimpleNamespace object if the response contains structured data.
        ...
        try:
            ...
            if reasoning and "<think>" in message and "</think>" in message:
                splitted_message = message.split("<think>")[1].split("</think>")
                if len(splitted_message) > 1:
                    return SimpleNamespace(reasoning=splitted_message[0], content=splitted_message[1])
                else:
                    return SimpleNamespace(reasoning=None, content=message)
            return message

        except (BadRequestError, APIError) as e:
            self._handle_api_error(logger, e)

    def chat_stream(self, message: str, images: List[str | bytes] = None, reasoning: bool = False) -> Iterator[str] | Iterator[SimpleNamespace]:
        ...
        Args:
            ...
            images (List[str | bytes]): Optional list of image file paths or raw bytes to include in the prompt.
            reasoning (bool): Whether to extract reasoning from the response if present.
                If True, the method will yield only the content part, excluding the reasoning.

        Yields:
            str | SimpleNamespace: Chunks of text (tokens) from the AI response
                or SimpleNamespace objects if reasoning is extracted.
            ...
            for chunk in super()._chat_stream_invoke(message=message, images=images):
                ...
                if in_thinking:
                    if "</think>" in chunk:
                        ...
                        reasoning_part, content_part = parts[0], parts[1]
                        if reasoning_part.strip():
                            yield SimpleNamespace(reasoning=reasoning_part, content=None)
                        if content_part.strip():
                            yield SimpleNamespace(reasoning=None, content=content_part)
                    else:
                        yield SimpleNamespace(reasoning=chunk, content=None)
                else:
                    if "<think>" in chunk:
                        in_thinking = True
                        parts = chunk.split("<think>", 1)
                        content_part, reasoning_part = parts[0], parts[1]
                        if content_part.strip():
                            yield SimpleNamespace(reasoning=None, content=content_part)
                        if reasoning_part.strip():
                            yield SimpleNamespace(reasoning=reasoning_part, content=None)
                    else:
                        yield SimpleNamespace(reasoning=None, content=chunk)
        except (BadRequestError, APIError) as e:
            self._handle_api_error(logger, e)
