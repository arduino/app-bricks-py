...
- `poses` (constructor) picks which built-in poses the instance listens to and tunes them one by one: a list of names, or of dicts with `name` plus `thresholds` ({"enter", "exit"}) and `smoothing` (seconds). The built-in poses left out stay in the classifier as negatives and never fire; `pose_names` holds the instance's vocabulary.
