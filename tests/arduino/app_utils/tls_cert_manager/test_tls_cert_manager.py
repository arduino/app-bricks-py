...
class TestBasicFunctionality:
    ...
    def test_reuse_existing_certificates(self, temp_certs_dir, reset_manager):
        ...
        cert_path1, key_path1 = TLSCertificateManager.get_or_create_certificates(
            certs_dir=temp_certs_dir
        )
        ...
        cert_path2, key_path2 = TLSCertificateManager.get_or_create_certificates(
            certs_dir=temp_certs_dir
        )
    ...
    def test_custom_validity_period(self, temp_certs_dir, reset_manager):
        ...
        cert_path, _ = TLSCertificateManager.get_or_create_certificates(
            certs_dir=temp_certs_dir,
            validity_days=1
        )
...
class TestPerformance:
    ...
    def test_concurrent_different_dirs_no_blocking(self, temp_certs_dir, reset_manager):
        ...
        assert overall_run_time < sum(elapsed_times), f"Bricks blocked each other: {overall_run_time:.3f}s should be lower than {sum(elapsed_times):.3f}s"
